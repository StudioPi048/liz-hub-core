...
export function MonthGrid({ currentDate, events, onDayClick, onEventClick, maxEventsPerDay = 3 }: MonthGridProps) {
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        ev.allDay ? "bg-primary/10 border-l-primary text-primary" : "bg-transparent border-l-muted-foreground text-foreground hover:bg-muted",
                        isConflict && "border-l-destructive bg-destructive/10 text-destructive font-medium"
                      )}
                      title={ev.title}
                    >
                      {!ev.allDay && <span className="font-medium mr-1">{format(new Date(ev.startsAt), "HH:mm")}</span>}
                      {ev.title}
                    </div>
                  );
                })}
