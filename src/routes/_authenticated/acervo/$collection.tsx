...
import { getTypeFromSlug, knowledgeTypeLabels, KnowledgeType } from "@/features/knowledge/model/knowledge-types";
  ...
  if (!type) {
    return (
      <div className="p-8 text-center">
        ...
        <Button asChild className="mt-4" variant="outline"><Link to="/acervo">Voltar ao Acervo</Link></Button>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" asChild>
          <Link to="/acervo"><ArrowLeft className="h-4 w-4" /></Link>
        </Button>
        <div>
      ...
      {isLoading ? (
        ...
        <div className="p-4 text-destructive border border-destructive/20 rounded-md bg-destructive/5">Erro ao carregar registros.</div>
      ) : data?.nodes.length === 0 ? (
        ...
        <>
          ...
          {data && data.totalPages > 1 && (
            <div className="flex justify-center gap-2 mt-8">
              ...
              <div className="flex items-center px-4 text-sm text-muted-foreground">Página {page} de {data.totalPages}</div>
...
function EmptyCollectionState({ type, label, hasSearch }: { type: KnowledgeType, label: string, hasSearch: boolean }) {
  if (hasSearch) {
    return (
  ...
  return (
    <div className="p-12 text-center border rounded-lg bg-muted/20 flex flex-col items-center">
      ...
      <p className="text-muted-foreground mt-2 max-w-md">
        Nenhum {type === 'person' || type === 'author' ? 'registro' : 'item'} foi cadastrado ainda no banco operacional.
      </p>
      ...
      <Card className="h-full overflow-hidden hover:border-primary/50 transition-colors flex flex-col">
        {hasCover ? (
          <div className="aspect-video w-full bg-muted border-b overflow-hidden relative">
            <img src={node.coverUrl} alt={node.title} className="object-cover w-full h-full opacity-90 group-hover:opacity-100 transition-opacity" />
          </div>
        ) : (
        ...
        <CardContent className="p-4 flex-1 flex flex-col">
          <div className="flex justify-between items-start gap-2 mb-2">
            <h3 className="font-semibold text-base line-clamp-2 leading-tight group-hover:text-primary transition-colors">{node.title}</h3>
          </div>
          <p className="text-sm text-muted-foreground line-clamp-3 mb-4 flex-1">
            {node.summary || "Sem descrição disponível."}
          </p>
          <div className="flex items-center gap-2 mt-auto">
            <Badge variant={node.status === 'approved' ? 'default' : 'secondary'} className="text-[10px] uppercase">
              {node.status}
            </Badge>
            {node.authority_level === 'official' && (
              <Badge variant="outline" className="text-[10px] border-blue-500/30 text-blue-600 bg-blue-50/50">Oficial</Badge>
            )}
          </div>
        </CardContent>
      </Card>
