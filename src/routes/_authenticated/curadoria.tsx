...
import { Loader2, FileText, CheckCircle2, AlertCircle, Sparkles, CheckSquare, Square, ChevronDown } from "lucide-react";
  ...
  const bulkApproveMutation = useMutation({
    mutationFn: async ({ ids, concept }: { ids: string[], concept: string }) => {
      ...
      const updates = currentNodes.map(node => {
        const newMetadata = { ...(node.metadata as any || {}) };
  ...
  return (
    <div className="space-y-6 max-w-full min-h-[calc(100vh-4rem)] p-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-editorial tracking-tight">
            Curadoria e Triagem
          </h1>
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 animate-in slide-in-from-bottom-5">
          <div className="bg-foreground text-background px-6 py-4 rounded-full shadow-2xl flex items-center gap-6">
          ...
          <div className="py-4 space-y-4">
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                Se os itens selecionados pertencerem ao mesmo tema, defina-o aqui para facilitar a estruturação futura da taxonomia.
              </p>
...
function KanbanColumn({ title, count, nodes, selectedIds, onToggle, onSelectAll, badgeVariant }: any) {
  const queryClient = useQueryClient();
    ...
    <div className="flex flex-col bg-muted/30 rounded-xl p-4 border border-border/40 min-h-[60vh]">
      <div className="flex items-center justify-between mb-4 px-1">
        <div className="flex items-center gap-2">
          ...
          <Badge variant={badgeVariant} className="text-xs">{count}</Badge>
        </div>
        <Button variant="ghost" size="sm" onClick={onSelectAll} className="h-8 px-2 text-xs text-muted-foreground">
          Selecionar Tudo
        </Button>
      </div>
      ...
      <div className="flex flex-col gap-3">
        {nodes.map((node: any) => {
          ...
          const concept = (node.metadata as any)?.primary_concept;
              ...
              <CardContent className="p-4 flex flex-col gap-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-medium text-sm leading-tight text-foreground line-clamp-2">{node.title}</p>
                ...
                <div className="border-t border-border/40 pt-3 mt-1 flex justify-between items-center" onClick={e => e.stopPropagation()}>
                  <Select 
                    value={concept || ""} 
                    onValueChange={async (val) => {
                      const newMeta = { ...(node.metadata || {}), primary_concept: val };
                      await supabase.from("knowledge_nodes").update({ metadata: newMeta }).eq("id", node.id);
