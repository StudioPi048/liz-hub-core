...
export const AssetSchema = z
  ...
  .refine(
    ...
    {
      message:
        "Invalid storage configuration for provider (e.g. supabase needs bucket/path and no url, external needs url and no bucket/path, repository needs path and no external url/bucket).",
    },
  )
  .refine(
    (data) => {
    ...
    { message: "Invalid combination of rights_status and visibility." },
  )
  .refine(
    (data) => {
      if (
        data.provider === "repository" &&
        (data.path?.includes("../") || data.path?.startsWith("/"))
      ) {
        return false;
      }
      return true;
