...
function generateInventory() {
  ...
  const allRelations = nodes.flatMap(n => (n.relations || []).map(r => ({ from: n.id, to: r.target, type: r.type })));
  ...
  const approvedOfficial = nodes.filter(n => n.status === 'approved' && n.authority_level === 'official').length;
  const draftUnverified = nodes.filter(n => n.status === 'draft' || n.authority_level === 'unverified').length;
